...
#main df read in 
df_all = pd.read_csv("../data/MHM_plus.csv",header=0)
...
#####################################################
#####################################################
fig1,ax1 = plt.subplots(nexps,3,figsize=[16,10]) #plot creation and config 
fig1.suptitle('MHM plus Data Dynamics') #full title config
...
for (e,ne) in zip(exps,range(nexps)):  #looping through each exp with number 
    ...
    ax1[ne,0].text(1.2,0.5,( str(e)),horizontalalignment='center', verticalalignment='center', transform=ax1[ne,2].transAxes)
    ax1[ne,0].semilogy()
...
fig1.savefig('../figures/MHM_plus_dynamics.png')
