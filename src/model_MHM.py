...
fig1.suptitle('MHM Dynamics') #full title config
...
fig2.suptitle('MHM Statistics') #full title config
...
#####################################################
fig3,ax3 = plt.subplots(nexps,3,figsize=[12,10]) #plot creation and config 
fig3.suptitle('MHM abiotic model dynamics and parameters') #full title config
...
for (e,ne) in zip(exps,range(nexps)):  #looping through each exp with number 
    ...
    inits0 = pd.read_csv("../data/inits/MHM0.csv") #use and updat inits 
...
#####################################################
    ...
    H0_prior=ODElib.parameter(stats_gen=scipy.stats.lognorm,\
                              hyperparameters={'s':(pw),'scale':10}) #setting state variiable  prior guess
...
# Set up graph for Dynamics and param histograms
    ...
    ax1[ne,0].plot(df0.time,df0.abundance, marker='o',label = 'MHM data ' + str(e) ) #data of 0 H assay
 ...
 ##########################################
    ax3[ne,0].plot(df0.time,df0.abundance, marker='o',label = 'MHM data ' + str(e) ) #data of 0 H assay
...
fig1.savefig('../figures/MHM_all_params.png')
fig2.savefig('../figures/MHM_all_stats.png')
fig3.savefig('../figures/MHM_all_dynamics.png')
